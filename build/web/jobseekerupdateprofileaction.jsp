...
String languagesknown=request.getParameter("languagesknown");
 ...
 ResultSet rs3=null;
  ...
  if(rs.next())
  {
	  ...
	  if(rs1.next())
	  {
          stmt.executeUpdate("update jobseekerprofile set username='"+username+"', fname='"+fname+"', lname='"+lname+"', designation='"+designation+"', location='"+location+"', experience='"+experience+"', email='"+email+"', mobile='"+mobile+"', keyskills='"+keyskills+"', projects='"+projects+"', itskills='"+itskills+"', education='"+education+"', certifications='"+certifications+"', workdetails='"+workdetails+"', dob='"+dob+"', gender='"+gender+"', hometown='"+hometown+"', maritialstatus='"+maritialstatus+"', languagesknown='"+languagesknown+"' where username='"+username+"' ");     // toLowerCase() is used to store data into table in LowerCase to remove case senstivity of fields except password. Password is case sensitive for security reasons.
	  }
	  else
	  {
	  stmt.executeUpdate("insert into jobseekerprofile values('"+username+"','"+fname+"','"+lname+"','"+designation+"','"+location+"','"+experience+"','"+email+"','"+mobile+"','"+keyskills+"','"+projects+"','"+itskills+"','"+education+"','"+certifications+"','"+workdetails+"','"+dob+"','"+gender+"','"+hometown+"','"+maritialstatus+"','"+languagesknown+"')");     // toLowerCase() is used to store data into table in LowerCase to remove case senstivity of fields except password. Password is case sensitive for security reasons.  
	  }
	  rs.close();
	  stmt.close();
	  con.close();
